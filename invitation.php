...
// Template file mapping - maps visual_style to template PHP files
$templateFiles = [
    ...
    'template-02' => 'templates/minimalis/index.php',
    'template-03' => 'templates/modern-emerald/index.php',
];

// If a non-default template is selected and its PHP file exists, include it and exit
if ($visual_style !== 'adat-jawa' && isset($templateFiles[$visual_style])) {
    $templatePath = $templateFiles[$visual_style];
    if (file_exists($templatePath)) {
        // Include template - it will have access to all variables defined above
        include $templatePath;
        exit;
    }
}

// If we reach here, use the default "adat-jawa" template below
...
<head>
    ...
    <title>The Wedding of <?= htmlspecialchars($invitation['groom_nickname']) ?> &
        <?= htmlspecialchars($invitation['bride_nickname']) ?>
    </title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link
        href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600;700&amp;family=Cormorant+Garamond:wght@400;500;600&amp;family=Poppins:wght@300;400;500&amp;display=swap"
        rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined" rel="stylesheet" />
    <link href="https://cdnjs.cloudflare.com/ajax/libs/simplelightbox/2.14.2/simple-lightbox.min.css"
        rel="stylesheet" />
    <style>
        body {
        ...
        .animate-fade-in-left {
            animation: fadeInLeft 1.2s cubic-bezier(0.2, 0.8, 0.2, 1) forwards;
            opacity: 0;
        }
...
<body class="h-full font-body scroll-smooth bg-[#FDF8F3] text-[#5D3A1A]">
    ...
    <div id="cover" class="fixed inset-0 z-50 flex items-center justify-center bg-cover bg-center overflow-hidden"
        ...
        <div class="absolute bottom-4 right-4 w-24 md:w-48 animate-fade-in-up pointer-events-none">
            <img src="<?= asset('assets/border-1.svg') ?>" class="w-full opacity-90 transform rotate-180 drop-shadow-lg"
                alt="Border">
        </div>

        <div
            class="text-center w-full max-w-2xl mx-auto relative z-10 px-6 flex flex-col items-center justify-center h-full">
            ...
            <div class="mb-8 animate-fade-in-up delay-300 relative py-12 md:py-16">
                ...
                <h1
                    class="font-display text-4xl md:text-6xl text-[#FDF8F3] leading-tight drop-shadow-lg relative z-10 px-4">
            ...
            <div class="space-y-4 mb-10 animate-fade-in-up delay-500 z-10 relative">
                <p
                    class="font-elegant text-xl md:text-2xl text-[#D4A855] border-y border-[#D4A855]/30 py-2 px-8 inline-block bg-black/20 backdrop-blur-sm rounded-full">
                    <?= date('l, d F Y', $akadDate) ?>
                </p>
            </div>

            <button id="open-invitation-btn"
                class="group relative px-8 py-3 rounded-full bg-[#D4A855] text-[#2c1810] font-bold tracking-wider transition-all hover:scale-105 hover:bg-[#eac47a] shadow-xl animate-fade-in-up delay-700 flex items-center gap-2 overflow-hidden z-20">
                <span
    ...
    <div id="app" class="h-full w-full overflow-auto hidden bg-fixed bg-cover bg-center"
        ...
        <!-- HERO -->
        <section id="hero"
            class="min-h-screen relative flex flex-col items-center justify-center py-16 md:py-20 px-4 overflow-hidden">
            <!-- Back Ornaments -->
            ...
            <div class="relative w-full max-w-3xl mx-auto text-center z-10">
                <p class="text-xs md:text-sm tracking-ultra mb-6 animate-on-scroll" data-animation="fade-in-down"
                    style="color: <?= $c['primary'] ?>;">THE WEDDING OF</p>

                <div class="space-y-4 mb-8">
                    <h1 class="font-display text-5xl md:text-7xl font-bold animate-on-scroll leading-tight"
                        data-animation="fade-in-left" style="color: <?= $c['secondary'] ?>;">
                        <?= htmlspecialchars($invitation['groom_nickname']) ?>
                    </h1>
                    ...
                    <h1 class="font-display text-5xl md:text-7xl font-bold animate-on-scroll leading-tight"
                        data-animation="fade-in-right" style="color: <?= $c['secondary'] ?>;">
                        <?= htmlspecialchars($invitation['bride_nickname']) ?>
                    </h1>
                </div>

                <div class="flex justify-center my-10 animate-on-scroll" data-animation="fade-in-up">
                    <img src="<?= asset('assets/ilustrasi-4.svg') ?>" class="w-32 md:w-48 animate-float drop-shadow-md"
                        alt="Mempelai">
                </div>
        ...
        <section id="couple" class="py-16 md:py-24 px-4 relative overflow-hidden">
            ...
            <div class="max-w-5xl mx-auto relative z-10 mt-6">
                ...
                <div class="grid md:grid-cols-2 gap-8 md:gap-20 items-stretch">
                    <!-- Groom -->
                    <div class="group relative text-center p-8 rounded-[2rem] bg-white shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-[#D4A855]/10 hover:border-[#D4A855]/30 transition-all duration-500 animate-on-scroll"
                        data-animation="fade-in-left">
                        <div
                            class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-[#FDF8F3] border border-[#D4A855]/20 rounded-full flex items-center justify-center text-[#D4A855] text-xl z-20 shadow-sm">
                            <span class="material-symbols-outlined">man</span>
                        </div>

                        <div class="relative w-48 h-48 mx-auto mb-6">
                            <div
                                class="absolute inset-0 rounded-full border-[3px] border-dashed border-[#D4A855]/40 animate-spin-slow">
                            </div>
                        ...
                        <h3 class="font-display text-3xl md:text-4xl mb-2 text-[#5D3A1A]">
                            <?= htmlspecialchars($invitation['groom_name']) ?>
                        </h3>
                        ...
                        <div class="text-sm text-[#8B4513]/80 space-y-1">
                            ...
                            <p class="font-bold text-lg text-[#5D3A1A]">Bapak
                                <?= htmlspecialchars($invitation['groom_father']) ?>
                            </p>
                            <p>&</p>
                            <p class="font-bold text-lg text-[#5D3A1A]">Ibu
                                <?= htmlspecialchars($invitation['groom_mother']) ?>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="group relative text-center p-8 rounded-[2rem] bg-white shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)] border border-[#D4A855]/10 hover:border-[#D4A855]/30 transition-all duration-500 animate-on-scroll"
                        ...
                        <div class="relative w-48 h-48 mx-auto mb-6">
                            <div class="absolute inset-0 rounded-full border-[3px] border-dashed border-[#D4A855]/40 animate-spin-slow"
                        ...
                        <h3 class="font-display text-3xl md:text-4xl mb-2 text-[#5D3A1A]">
                            <?= htmlspecialchars($invitation['bride_name']) ?>
                        </h3>
                        ...
                        <div class="text-sm text-[#8B4513]/80 space-y-1">
                            <p>Putri dari Pasangan</p>
                            <p class="font-bold text-lg text-[#5D3A1A]">Bapak
                                <?= htmlspecialchars($invitation['bride_father']) ?>
                            </p>
                            <p>&</p>
                            <p class="font-bold text-lg text-[#5D3A1A]">Ibu
                                <?= htmlspecialchars($invitation['bride_mother']) ?>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <section id="story" class="py-16 md:py-24 px-4 bg-[#D4A855]/5 border-y border-[#D4A855]/20">
                <div class="max-w-4xl mx-auto">
                    ...
                    <div class="relative pl-8 md:pl-0 max-w-3xl mx-auto">
                        <!-- Vertical Line -->
                        <div
                            class="absolute left-8 md:left-12 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-[#D4A855] to-transparent">
                        </div>
                        ...
                        <div class="space-y-16">
                            <?php foreach ($stories as $story): ?>
                                <div class="relative pl-8 md:pl-16 group animate-on-scroll" data-animation="fade-in-up">
                                    <!-- Dot -->
                                    <div class="absolute left-6 md:left-10 top-0 w-5 h-5 rounded-full border-4 border-[#FDF8F3] bg-[#D4A855] z-10 shadow-md group-hover:scale-125 transition-transform duration-300"
                                        style="transform: translateX(-50%);"></div>

                                    <!-- Content -->
                                    <div
                                        class="bg-white p-8 rounded-2xl shadow-sm border border-[#D4A855]/10 hover:shadow-lg transition-all duration-300 relative">
                                        <span
                                            class="absolute -left-2 top-4 w-4 h-4 bg-white transform rotate-45 border-l border-b border-[#D4A855]/10"></span>
                                        <div class="flex flex-col md:flex-row md:items-center gap-4 mb-4">
                                            <span
                                                class="inline-block px-4 py-1 rounded-full bg-[#D4A855]/10 text-[#D4A855] font-bold text-sm tracking-widest border border-[#D4A855]/20">
            ...
            <div class="max-w-5xl mx-auto relative z-20">
                <div class="text-center mb-16 animate-on-scroll" data-animation="fade-in-down">
                    ...
                    <p class="font-elegant text-lg text-[#5D3A1A]/70 max-w-2xl mx-auto">
                        Dengan segala kerendahan hati, kami mengundang Bapak/Ibu/Saudara/i untuk hadir dalam acara
                        pernikahan kami:
                    </p>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-8 md:gap-12">
                    <!-- Akad -->
                    <div class="group relative text-center px-8 py-12 rounded-[2rem] bg-white border border-[#D4A855]/20 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_40px_-10px_rgba(212,168,85,0.1)] transition-all duration-500 animate-on-scroll"
                        data-animation="fade-in-left">
                        <!-- Orn -->
                        <div class="absolute top-4 left-4 w-12 opacity-20 pointer-events-none"><img
                                src="<?= asset('assets/border-1.svg') ?>"></div>
                        <div class="absolute bottom-4 right-4 w-12 opacity-20 pointer-events-none rotate-180"><img
                                src="<?= asset('assets/border-1.svg') ?>"></div>

                        <div
                            class="w-20 h-20 mx-auto mb-6 rounded-full flex items-center justify-center bg-[#FDF8F3] border border-[#D4A855] text-[#D4A855] group-hover:scale-110 transition-transform duration-500 shadow-inner">
                        ...
                        <div class="space-y-4 mb-8">
                            <div>
                                <p class="text-xs font-bold tracking-widest text-[#D4A855] uppercase mb-1">Hari /
                                    Tanggal</p>
                            ...
                            <div>
                                ...
                                <p class="font-elegant text-2xl text-[#5D3A1A]"><?= date('H:i', $akadDate) ?> WIB -
                                    Selesai</p>
                            </div>
                            <div>
                                <p class="text-xs font-bold tracking-widest text-[#D4A855] uppercase mb-1">Lokasi</p>
                                <p class="text-sm md:text-base text-[#5D3A1A]/80 px-4">
                                    <?= htmlspecialchars($invitation['event_address']) ?>
                                </p>
                            </div>
                        </div>

                        <?php if ($akadMap): ?>
                            <a href="<?= htmlspecialchars($akadMap) ?>" target="_blank"
                                class="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-[#5D3A1A] text-white text-sm font-medium transition hover:bg-[#D4A855] hover:text-[#5D3A1A] shadow-lg">
                    ...
                    <div class="group relative text-center px-8 py-12 rounded-[2rem] bg-white border border-[#D4A855]/20 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_40px_-10px_rgba(212,168,85,0.1)] transition-all duration-500 animate-on-scroll"
                        ...
                        <div class="space-y-4 mb-8">
                            ...
                            <div>
                                <p class="text-xs font-bold tracking-widest text-[#D4A855] uppercase mb-1">Waktu</p>
                                <p class="font-elegant text-2xl text-[#5D3A1A]"><?= date('H:i', $recepDate) ?> WIB -
                                    Selesai</p>
                            </div>
                            <div>
                                <p class="text-xs font-bold tracking-widest text-[#D4A855] uppercase mb-1">Lokasi</p>
                                <p class="text-sm md:text-base text-[#5D3A1A]/80 px-4">
                                    <?= htmlspecialchars($invitation['reception_address']) ?>
                                </p>
                            </div>
                        </div>

                        <?php if ($recepMap): ?>
                            <a href="<?= htmlspecialchars($recepMap) ?>" target="_blank"
                                class="inline-flex items-center gap-2 px-8 py-3 rounded-full bg-[#5D3A1A] text-white text-sm font-medium transition hover:bg-[#D4A855] hover:text-[#5D3A1A] shadow-lg">
                                <span class="material-symbols-outlined text-lg">map</span>
                                Google Maps
                            </a>
        ...
        if ((is_array($gifts) && count($gifts) > 0) || !empty($invitation['enabled_features']) && in_array('gift', $features)):
            ...
            <section id="gift" class="py-24 px-4 bg-[#FDF8F3]">
                <div class="max-w-2xl mx-auto text-center">
                    <div class="inline-block p-3 rounded-full bg-[#D4A855]/10 text-[#D4A855] mb-6 animate-on-scroll"
                        data-animation="zoom-in">
                        <span class="material-symbols-outlined text-3xl">volunteer_activism</span>
                    </div>
                    <h2 class="font-display text-4xl md:text-5xl mb-6 animate-on-scroll text-[#5D3A1A]"
                        data-animation="fade-in-down">Teddy Gift</h2>
                    <p class="mb-12 font-elegant text-xl animate-on-scroll text-[#5D3A1A]/70 leading-relaxed"
                        data-animation="fade-in">
                        Doa restu Anda merupakan karunia yang sangat berarti bagi kami. Dan jika memberi adalah ungkapan
                        tanda kasih Anda, Anda dapat memberi kado secara cashless.
                    </p>
                    ...
                    <div class="space-y-6 animate-on-scroll" data-animation="fade-in-up">
                        <?php if (is_array($gifts)):
                            foreach ($gifts as $gift): ?>
                                <div
                                    class="p-8 rounded-2xl bg-white shadow-lg border border-[#D4A855]/20 flex flex-col items-center gap-4 relative overflow-hidden group hover:-translate-y-2 transition-transform duration-300">
                                    <!-- Decorative bg -->
                                    <div
                                        class="absolute -right-10 -bottom-10 w-32 h-32 bg-[#D4A855]/5 rounded-full pointer-events-none">
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2 mb-2">
                                        <?php if (isset($gift['logo']) && in_array($gift['logo'], ['bca', 'bri', 'mandiri', 'bni', 'cimb', 'dana', 'ovo', 'gopay', 'shopeepay', 'linkaja'])): ?>
                                            <span
                                                class="font-bold text-2xl uppercase tracking-wider text-[#5D3A1A]"><?= $gift['logo'] ?></span>
                                        <?php else: ?>
                                    ...
                                    <div class="text-center z-10">
                                        <p
                                            class="text-3xl font-mono font-bold tracking-widest my-2 select-all text-[#D4A855] drop-shadow-sm">
                                            <?= htmlspecialchars($gift['number']) ?>
                                        </p>
                                        <p class="text-sm font-bold text-[#5D3A1A] uppercase tracking-wide">a.n
                                    ...
                                    <!-- Copy Button -->
                                    <button
                                        onclick="navigator.clipboard.writeText('<?= htmlspecialchars($gift['number']) ?>'); alert('Nomor Rekening Disalin!');"
                                        class="absolute right-4 top-4 p-2 rounded-full hover:bg-gray-100 text-[#D4A855] transition transform hover:rotate-12">
        ...
        <?php if (in_array('countdown', $features) && $akadDate): ?>
            <section id="countdown" class="py-16 md:py-24 px-4 bg-fixed bg-cover bg-center relative"
                    ...
                    <div class="grid grid-cols-4 gap-4 md:gap-8 animate-on-scroll" data-animation="zoom-in">
                        <?php foreach (['Hari', 'Jam', 'Menit', 'Detik'] as $i => $label): ?>
                            <div class="p-6 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 shadow-xl">
                                <span id="cd-<?= $i ?>"
                                    class="font-display text-4xl md:text-6xl block text-[#D4A855] text-shadow">00</span>
                                <span
                                    class="text-xs md:text-sm font-bold tracking-widest uppercase text-white/80 mt-2 block"><?= $label ?></span>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 animate-on-scroll" data-animation="zoom-in">
                        <?php foreach ($gallery_links as $link): ?>
                            <a href="<?= htmlspecialchars(trim($link)) ?>"
                                class="gallery-item block aspect-square rounded-xl overflow-hidden shadow-md relative group">
                                <img src="<?= htmlspecialchars(trim($link)) ?>"
                                    class="w-full h-full object-cover transition duration-700 group-hover:scale-110">
                                <div
                                    class="absolute inset-0 bg-[#5D3A1A]/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center text-white backdrop-blur-[2px]">
                                    <span
                                        class="material-symbols-outlined text-4xl drop-shadow-md transform scale-0 group-hover:scale-100 transition-transform duration-300 delay-100">zoom_in</span>
                                </div>
                            </a>
                        <?php endforeach; ?>
                    </div>
        ...
        <?php if (in_array('rsvp', $features)): ?>
            <section id="rsvp" class="py-16 md:py-24 px-6 bg-[#FDF8F3] border-t border-[#D4A855]/20">
                <div class="max-w-lg mx-auto text-center">
                    <h2 class="font-display text-4xl md:text-5xl mb-8 animate-on-scroll text-[#5D3A1A]"
                        data-animation="fade-in-down">Konfirmasi Kehadiran</h2>

                    <form action="guest_handler.php" method="POST" class="space-y-6 animate-on-scroll"
                        ...
                        <div class="relative group">
                            <span
                                class="material-symbols-outlined absolute left-4 top-3.5 text-[#D4A855] group-focus-within:text-[#5D3A1A] transition-colors">person</span>
                            <input type="text" name="name" placeholder="Nama Lengkap" required
                                class="w-full pl-12 pr-4 py-3 rounded-xl border border-[#D4A855]/30 bg-white focus:outline-none focus:ring-2 focus:ring-[#D4A855]/50 focus:border-[#D4A855] transition-all shadow-sm">
                        </div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex-1 cursor-pointer group">
                                ...
                                <div
                                    class="p-4 rounded-xl border border-[#D4A855]/30 bg-white transition-all peer-checked:bg-[#D4A855] peer-checked:text-white peer-checked:shadow-lg hover:bg-gray-50 flex flex-col items-center gap-1">
                            ...
                            <label class="flex-1 cursor-pointer group">
                                ...
                                <div
                                    class="p-4 rounded-xl border border-[#D4A855]/30 bg-white transition-all peer-checked:bg-[#5D3A1A] peer-checked:text-white peer-checked:shadow-lg hover:bg-gray-50 flex flex-col items-center gap-1">
                        ...
                        <div class="relative group">
                            <span
                                class="material-symbols-outlined absolute left-4 top-3.5 text-[#D4A855] group-focus-within:text-[#5D3A1A] transition-colors">edit_note</span>
                            <textarea name="message" placeholder="Ucapan & Doa..." rows="3"
                                class="w-full pl-12 pr-4 py-3 rounded-xl border border-[#D4A855]/30 bg-white focus:outline-none focus:ring-2 focus:ring-[#D4A855]/50 focus:border-[#D4A855] transition-all shadow-sm resize-none"></textarea>
                        </div>

                        <button type="submit"
                            class="w-full py-4 rounded-full font-bold text-white transition-all transform hover:scale-[1.02] active:scale-95 shadow-lg flex items-center justify-center gap-2"
                            style="background: linear-gradient(to right, #8B4513, #5D3A1A);">
                    ...
                    <div class="mt-16 text-left space-y-4 max-h-[500px] overflow-y-auto px-2 custom-scrollbar">
                        ...
                        <?php foreach ($wishes as $wish): ?>
                            <div class="p-4 rounded-xl bg-white shadow-sm border border-[#D4A855]/10 relative">
                                <div class="absolute top-4 right-4 text-[#D4A855]/20"><span
                                        class="material-symbols-outlined text-4xl">format_quote</span></div>
                                ...
                                <p class="text-sm italic text-[#5D3A1A]/80 pl-4 border-l-2 border-[#D4A855]/20">
                                    "<?= htmlspecialchars($wish['message']) ?>"</p>
                                <p class="text-xs text-right mt-2 text-gray-400">
        ...
        <footer class="py-16 px-4 text-center text-[#FDF8F3] relative overflow-hidden" style="background: #5D3A1A;">
            <div class="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none"
                style="background-image: url('<?= asset('assets/ilustrasi-1.svg') ?>'); background-size: 400px; background-repeat: repeat;">
            </div>
            ...
            <div class="relative z-10">
                ...
                <p class="text-[0.65rem] text-white/40 tracking-widest uppercase mb-4">&copy; <?= date('Y') ?>
    ...
    <!-- NAVBAR -->
    <div id="navbar"
        class="fixed bottom-6 left-1/2 transform -translate-x-1/2 z-40 px-6 py-3 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex gap-6 items-center transition-all duration-500 translate-y-24 opacity-0 bg-white/80 backdrop-blur-md border border-white/50">
        <button onclick="scrollToSection('hero')"
            class="text-2xl text-[#5D3A1A] hover:text-[#D4A855] hover:-translate-y-1 transition-all nav-btn">
    ...
    <?php if (!empty($music_file)): ?>
        ...
        <button id="music-toggle"
            class="hidden fixed bottom-24 right-6 z-40 w-12 h-12 rounded-full shadow-lg flex items-center justify-center bg-white/90 backdrop-blur border border-[#D4A855]">
